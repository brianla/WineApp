...
def dbGetUserShelves(user):
  ...
  Return:
    recommenders: A list of shelves the user has
  """
  err = []
  sysErr = []
  recommenders = []
...
def dbSetCSOByWineID(wineID, CSO):
  ...
  err = []
  sysErr = []
  print "wineID for CSO setting is: " + str(wineID)
  print "CSO is " + str(CSO)
...
def dbShelfWineCandidates (wineDict):
  ...
  Args: 
    dict: Dictionary of attribute (wineName, winery, vintage)

  Return:
    wineList: List of wines
  """
  err = []
  sysErr = []
  wineList = []
  if not (wineDict['wineName'] == '' and wineDict['winery'] == '' and wineDict['vintage'] == ''):
    try:
      ...
      #print 'Execute WURKED'
      #wines = DBAccessConnection.con.execute('SELECT * FROM Wines WHERE wineName LIKE \"%' + wineDict['wineName'] + '%\" AND winery LIKE \"%' + wineDict['winery'] + '%\" AND vintage LIKE \"%' + wineDict['vintage'] + '%\";')
...
def dbGetWinesByAttr(attributes):
  ...
  Possibly outdated, do not implement
  """
...
def dbGetRecommenderHistory(channelID):
  ...
  Args: 
    channelID: ID of the channel whose recommender history we want to get

  Return: 
    recommendedList: A list of RecommenderHistory objects
  """
  #userID = request.cookies.get('username')
  #locations = locationMap.select(locationMap.c.lm_userID == userID).execute()
  #locations = DBAccessConnection.con.execute(locationMap.select(locationMap.c.lm_userID == userID))
  err = []
  sysErr = []
  recommendedList = []
  try:
...
def dbGetWinesByName(name):
  ...
  Return:
    wines: The list of wines with the name specified
  """
  err = []
  sysErr = []
  wines = []
...
def dbAddWineGlobal(wine):
  ...
  try:
    ...
    wine2 = DBAccessConnection.con.execute('SELECT * FROM Wines w WHERE w.wineID NOT IN ( SELECT w2.wineID FROM Wines w2, Wines w3 WHERE w2.wineID < w3.wineID);').first()
    #wine2 = DBAccessConnectionn.con.execute(" + ' AND w.winery = ' + "\"" + str(wine.winery) + "\"" + ' AND w.vintage = ' + str(wine.vintage) +';').first()
...
def dbGetWineIDs():
  ...
  Return:
    wineIDlist: List containing all wineIDs in the Wines table
...
def dbRandomWine():
  ...
  Return:
    randwine: A random wine from the Wines table
  """
  err = []
  sysErr = []
  try:
  ...
  try:
    #rwine = Wines.select(Wines.c.wineID == randomID).execute()
...
def dbGetFeatureVector():
  ...
  Return:
    Wine referenceWine: The wine with wineID = 0
  """
...
def dbGetSeeds(channelID):
  ...
  try:
    ...
    for wine in seeds:
      #print "This wine is " + str(wine)
      #print "This wine's items are " + str(wine.items())
      ...
      # The below call doesn't work because we have a recommender history row 
      # proxy not a wine row proxy
      #seed = Wine(wine['rh_wineID'],wine['wineName'],wine['varietal'],wine['winery'],wine['wineType'],wine['vintage'],wine['region'],wine['clusterID'],wine['CSO'],wine['tags'],wine['description'],wine['averageStarRating'],wine['imagePath'],wine['barcode'],wine['bitter'],wine['sweet'],wine['sour'],wine['salty'],wine['chemical'],wine['pungent'],wine['oxidized'],wine['microbiological'],wine['floral'],wine['spicy'],wine['fruity'],wine['vegetative'],wine['nutty'],wine['caramelized'],wine['woody'],wine['earthy'])
