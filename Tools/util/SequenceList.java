...
public class SequenceList {
    ...
    public SequenceList(String range) {
        ptr = 0;
     ...
     */    
    public  IndexedHashMap<Integer> expand(String ranges, int maxNumber) {
    	//System.err.println("Try to expand ranges from this string:'" + ranges + "'");
        SequenceList parse = new SequenceList(ranges);
        IndexedHashMap<Integer> list = new IndexedHashMap<Integer>();
        boolean sair = false;
        while (!sair) {
            ...
            if (parse.inverse) {
                ...
                */
                ...
                for (int z = 0; z < list.length(); z++) {
                    int n = ((Integer)list.get(z)).intValue();
                    if (parse.even && (n & 1) == 1)
                        continue;
                    if (parse.odd && (n & 1) == 0)
                        continue;
                    if (n >= parse.low && n <= parse.high)
                        list.removeByIndex(z);
                }
            }
            else {
                if (parse.low > parse.high) {
